...
具体的には「次の何かは繰り返しのタスク」として、Google スプレッドシートに記述した `HH:MM` のルール等を元に特定期間の開始時刻をすべて計算し、Notion データベースへページとして送信するライブラリーです。
...
「次」を確認したくなる例として、以下のようなケースが考えられます。

### ダッシュボード風に時刻表表示

データベースに保存された発車時刻のページをビューで表示することで、ダッシュボード風に時刻表を表示できます。

![Notion 上で直近の時刻表を 2 つ表示しているスクリーンショット](images/next-nanika.png)

### Notion カレンダーで時間割表示

休日は Google カレンダー にイベントを設定することで指定できます。

![Notion カレンダー上で時間割を表示しているスクリーンショット](images/next-nanika-calendar.png)
...
`NextNanika.run()` は時刻表寄りに作ってあるので少し設定が面倒ですが、学期毎に一括作成しておくと運用しやすいかなと思います。

- `startDaysOffset` と `daysToProcess` で作成期間を指定(値は関数実行日からの相対日です)
- `skipCleanup: true` で cleanup 処理をスキップ(作成したページにモなどを記述するときは設定しておくとページが削除されません)
...
### 名前(タイトル)にタイムスタンプを含める

Notion のページタイトルに `@` によるタイムスタンプを含める場合、名前の値に `@` で始まるキーワードを記述します。

- `@time` : 開始時刻と終了時刻(設定されている場合)がセットされる
- `@startTime` : 開始時刻がセットされる
- `@date` : 開始時刻と終了時刻(設定されている場合)の日付がセットされる
- `@startDate` : 開始時刻の日付がセットされる

なお、以下の制限があります。

- Notion 上での表示形式などは指定できません
- 重複チェックのときに名前(タイトル)の比較は行われなくなります(名前は一致していると判定されます)
