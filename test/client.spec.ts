...
describe('Client', () => {
  ...
  it('should createPage is successful', async () => {
    const mockfetch = jest.fn().mockReturnValue({
      getResponseCode: jest.fn().mockReturnValue(200),
      getContentText: jest
        .fn()
        .mockReturnValue(JSON.stringify(exampleCreatePageResult))
    })
    global.UrlFetchApp = {
      fetch: mockfetch
    } as any
    ...
    const payload: Parameters<Client['createPage']>[0] = {
      parent: {
        type: 'database_id',
    ...
    await expect(c.createPage(payload)).resolves.toEqual(
      exampleCreatePageResult
    )
  ...
  it('should reject in createPage by internal server error', async () => {
    const mockfetch = jest.fn().mockReturnValue({
      getResponseCode: jest.fn().mockReturnValue(500),
      getContentText: jest.fn().mockReturnValue('Internal Server Error')
    })
    ...
    await expect(c.createPage(payload)).rejects.toThrow(
      /^createPage 500, text: Internal Server Error$/
    )
  })

  it('should trashPage is successful', async () => {
    const mockfetch = jest.fn().mockReturnValue({
      getResponseCode: jest.fn().mockReturnValue(200),
      getContentText: jest
        .fn()
    ...
    await expect(c.trashPage(pageId)).resolves.toEqual(exampleCreatePageResult)
    expect(mockfetch).toHaveBeenCalledWith(
  ...
  it('should reject in trashPage by internal server error', async () => {
    const mockfetch = jest.fn().mockReturnValue({
      getResponseCode: jest.fn().mockReturnValue(500),
      getContentText: jest.fn().mockReturnValue('Internal Server Error')
    })
    ...
    const pageId = randomUUID()
    await expect(c.trashPage(pageId)).rejects.toThrow(
      /^trashPage 500, text: Internal Server Error$/
    )
  })
})
